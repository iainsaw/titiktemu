...
const baseNames = new Set(
  files.filter((f) => f.endsWith(".shp")).map((f) => f.replace(".shp", "")),
);
...
try {
  ...
  execSync(
    `npx -y mapshaper -i ${BUS_DIR}/*.shp combine-files -merge-layers force -filter-fields name,ref -simplify dp 5% -o public/rute-bus-bandung.geojson format=geojson`,
    { stdio: "inherit" },
  );

  console.log("Generating rute-angkot-bandung-micro.geojson...");
  execSync(
    `npx -y mapshaper -i ${ANGKOT_DIR}/*.shp combine-files -merge-layers force -filter-fields name,ref -simplify dp 5% -o public/rute-angkot-bandung-micro.geojson format=geojson`,
    { stdio: "inherit" },
  );
