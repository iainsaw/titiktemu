      ...
      fetchPromise = (async () => {
        try {
          const [{ data, error }, geoRes] = await Promise.all([
            ...
            getSecureAssetUrl("datasetfix.geojson").then((url) =>
              url ? fetch(url).catch(() => null) : null,
            ),
          ]);
          ...
          const baseIds = Array.from(
            { length: 16 },
            (_, i) => `KWS-${String(i + 1).padStart(2, "0")}`,
          );
          ...
          const parsedData = deduplicatedData.map((dbData) => {
            ...
            if (coord && gridFeatures.length > 0) {
              ...
              if (gridsInRadius > 0) {
                ...
                kws.skor.layanan = Math.min(
                  100,
                  Math.max(1, Math.round((sumFasilitas * 100) / 150)),
                ); // benchmark 150 POI
